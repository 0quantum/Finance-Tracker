...
function ChangePasswordCard() {
  ...
  return (
    <Card className="p-5 rounded-2xl border bg-white dark:bg-muted shadow-sm">
      ...
      <p className="text-xs text-muted-foreground mb-4">Choose a new password for your account</p>

      <FieldGroup>
        <Field>
          <FieldLabel htmlFor="new-password">New password</FieldLabel>
          <Input id="new-password" type="password" placeholder="Min. 6 characters" value={password} onChange={(e) => setPassword(e.target.value)} />
        </Field>
        <Field>
          <FieldLabel htmlFor="confirm-password">Confirm password</FieldLabel>
          <Input id="confirm-password" type="password" placeholder="Repeat your password" value={confirm} onChange={(e) => setConfirm(e.target.value)} />
        </Field>
...
function ThemeCard() {
  // lazy initializer — runs only on client, avoids useEffect setState warning
  ...
  return (
    <Card className="p-5 rounded-2xl border bg-white dark:bg-muted shadow-sm">
      ...
      <p className="text-xs text-muted-foreground mb-4">Choose how FinanceApp looks for you</p>
...
function DangerZoneCard() {
  ...
  const handleDelete = async () => {
    if (confirm !== PHRASE) { setError(`Type "${PHRASE}" to confirm.`); return; }
    setError(null);
    setLoading(true);
